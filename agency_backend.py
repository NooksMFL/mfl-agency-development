...
def extract_list(d):
 ...
 if isinstance(d,dict):
  ...
  if isinstance(d.get("data"),dict): return extract_list(d["data"])
 return None
def roster(wallet):
 # Public wallet-first lookup. Try the query shapes used by MFL/community tooling.
 attempts=[
  ("/players",{"ownerWalletAddress":wallet,"limit":1500}),
  ("/players",{"walletAddress":wallet,"limit":1500}),
  ("/players",{"owner":wallet,"limit":1500}),
 ]
 last=None
 for path,params in attempts:
  r=requests.get(BASE+path,params=params,headers=H,timeout=30); last=r
  if r.ok:
   xs=extract_list(r.json())
   if xs is not None: return xs
 if last is not None: last.raise_for_status()
 raise RuntimeError("MFL roster endpoint did not return a player list")
def pick(p,*ks):
 for k in ks:
...
def norm(p):
 ...
 age=pick(p,"age") or m.get("age"),position=pos,club=club or "",ovr=pick(p,"overall","overallRating") or a.get("overall"),
 pac=pick(p,"pace") or a.get("pace"),sho=pick(p,"shooting") or a.get("shooting"),pas=pick(p,"passing") or a.get("passing"),
 dri=pick(p,"dribbling") or a.get("dribbling"),def_=pick(p,"defense","defending") or a.get("defense") or a.get("defending"),phy=pick(p,"physical") or a.get("physical"))
def sync_wallet(wallet):
 wallet=wallet.strip().lower(); ps=[norm(x) for x in roster(wallet)]; now=datetime.now(timezone.utc).isoformat()
 c=db();init(c);c.execute("INSERT OR IGNORE INTO wallets VALUES(?,?)",(wallet,now))
 openp={r["player_id"]:r for r in c.execute("SELECT * FROM ownership WHERE wallet=? AND ended_at IS NULL",(wallet,))}
 ...
 for pid in set(openp)-ids:c.execute("UPDATE ownership SET ended_at=? WHERE wallet=? AND player_id=? AND ended_at IS NULL",(now,wallet,pid))
